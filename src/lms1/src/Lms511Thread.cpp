...
Lms511Thread::Lms511Thread(void):
    m_strNodeName("Lms1Driver"),
    m_strTopicName("lms1Scan"),
    m_strFrameID("lms1")
{
    m_pMsg = new char[50];
    ...
    m_tcpSocketLms511.connectToHost("192.168.0.32", 2112);
    //connect(&m_tcpSocketLms511, SIGNAL(connected()), this, SLOT(StartMeasurements()));
    connect(&m_tcpSocketLms511, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(Err()));
}
...
{
    int indexOfHead = -1;
    //cout<<"m_pContinuousScandataHeader="<<m_pContinuousScandataHeader<<endl;
    ...
    cout<<"m_Frame.data="<<m_Frame.data()<<endl;
